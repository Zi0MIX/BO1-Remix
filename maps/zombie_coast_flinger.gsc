...
init_flinger()
{
    ...
    level thread maps\_remix_coast_flinger::start_flinger_in_open_position();

    level thread maps\_remix_coast_flinger::flinger_think();
...
player_launch(flipper_area, gate)
{
    ...
    for(i = 0; i < players.size; i++)
    {
        ...
        {
            players[i] thread maps\_remix_coast_flinger::fling_player(launch_spots[i]);
        }
    }
...
boss_launch(flipper_area)
{
    ...
    if ( self istouching(flipper_area) )
    {
        ...
        self thread maps\_remix_coast_flinger::unlink_later(org1 );
    }
...
do_flipper_corpse_cleanup(area)
{
    ...
    if(IsDefined(corpses))
    {
        for ( i = 0; i < corpses.size; i++ )
        {
            ...
            {
                corpses[i] thread maps\_remix_coast_flinger::remove_corpse();
            }
        }
    }
}
